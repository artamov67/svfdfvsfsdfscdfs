...
API_TOKEN = "8704894935:AAFEuhM29EHTgvd6FskbDFvkt65UNabtt2k"  # Например: "1234567890:ABCdefGHIjklMNOpqrsTUVwxyz"
...
async def about_access(callback: CallbackQuery):
    await callback.message.edit_text(
        ...
        "• Пресеты\n"
        "• Поддержка и помощь\n"
        "• Больше информации тут - ссылку потом добавлю\n\n"
        f"Цена: {get_current_price()} ⭐\n"
        "После оплаты вы получите одноразовую ссылку для входа.",
        reply_markup=get_main_keyboard()
    )
    await callback.answer()
...
async def successful_payment(message: Message):
    ...
    try:
        ...
        await message.answer(
            "❌ Произошла ошибка при создании ссылки. Пожалуйста, обратитесь к администратору."
        )
